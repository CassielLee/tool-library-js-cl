...
const format = (value, format = "yyyy-MM-dd") => {
  ...
  str = str.replace(
    /yy|YY/,
    date.getYear() % 100 > 9 ? date.getYear() % 100 : `0${date.getYear() % 100}`
  );
...
const dateTransfrom = date => {
  ...
  const year = date.split("年");
  const month = year[1].split("月");
  const final_month = monthArr[parseInt(month[0]) - 1];
  const day = month[1].slice(0, -1);
  const final_date = final_month + " " + day + "," + year[0];
  return final_date;
};

export { time, format, formatSecond, dateTransfrom };
